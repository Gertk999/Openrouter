...
// The `messages` state array accumulates UI-only entries alongside real
// conversation turns -- `role: 'compare'` (holds a `results` array, not
// `content`) and `role: 'error'` (a client-side error bubble). Sending the
// raw array straight to OpenRouter includes those and produces messages
// with `content: undefined`, which the API rejects with a 400
// ("text content parts must carry a string \"text\"", got undefined).
// This keeps only real user/assistant turns for the API call.
